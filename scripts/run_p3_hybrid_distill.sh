...
# One-member pilot/full runner for P3 Hybrid + logit/feature distillation.
# It never starts member 1/2 and never overwrites an existing output namespace.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
PYTHON_BIN="${P3D_PYTHON:-python}"
GPU="${P3D_GPU_ID:-0}"
CONFIG="${P3D_CONFIG:-$ROOT/configs/p3_hybrid_distill_full8.json}"
OUT="${P3D_OUT:-$ROOT/outputs/p3_hybrid_distill_full8_seed0}"
MONITOR="${P3D_MONITOR:-$ROOT/outputs/p3_hybrid_distill_monitor}"
...
find_fold_root() {
  if [[ -n "${P3D_FOLD_ROOT:-}" ]]; then printf '%s\n' "$P3D_FOLD_ROOT"; return; fi
  ...
  (( ${#found[@]} == 1 )) || die "Set P3D_FOLD_ROOT to the directory containing fold_assignments.parquet and fold_manifest.json"
  printf '%s\n' "${found[0]}"
}

find_prep_root() {
  if [[ -n "${P3D_PREP_ROOT:-}" ]]; then printf '%s\n' "$P3D_PREP_ROOT"; return; fi
  ...
  (( ${#found[@]} == 1 )) || die "Set P3D_PREP_ROOT to the preprocessing root containing member_0/1/2/B/fold_preprocessing.json"
  printf '%s\n' "${found[0]}"
}
...
dry_run() {
  check
  "$PYTHON_BIN" src/training/fold_ensemble3_full.py "${COMMON[@]}" --member-id 0
}
...
start() {
  ...
  check
  if [[ -e "$OUT/member_${member}" ]]; then
    echo "[INFO] Existing member namespace found; orchestrator will skip or resume it without overwriting completed artifacts."
  fi
  if [[ "$member" == 1 ]]; then
    [[ -s "$OUT/member_0/checkpoints/task_7.pt" && -s "$OUT/member_0/task_7/completed_task.json" ]] \
      || die "Member 0 chưa hoàn tất task 7."
  elif [[ "$member" == 2 ]]; then
    [[ -s "$OUT/member_1/checkpoints/task_7.pt" && -s "$OUT/member_1/task_7/completed_task.json" ]] \
      || die "Member 1 chưa hoàn tất task 7."
  fi
  mkdir -p "$MONITOR"
  local stamp log
