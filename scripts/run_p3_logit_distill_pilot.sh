...
# One-member P3 pilot: best hybrid settings plus LOGIT distillation only.
# Feature distillation is disabled deliberately for a clean ablation.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
PYTHON_BIN="${LOGIT_DISTILL_PYTHON:-python}"
GPU="${LOGIT_DISTILL_GPU_ID:-0}"
CONFIG="${LOGIT_DISTILL_CONFIG:-$ROOT/configs/p3_hybrid_logit_distill_full8_e30_mem4_pilot.json}"
OUT="${LOGIT_DISTILL_OUT:-$ROOT/outputs/p3_hybrid_logit_distill_full8_e30_mem4_pilot_seed0}"
MONITOR="${LOGIT_DISTILL_MONITOR:-$ROOT/outputs/p3_logit_distill_pilot_monitor}"
...
find_fold_root() {
  if [[ -n "${LOGIT_DISTILL_FOLD_ROOT:-}" ]]; then
    printf '%s\n' "$LOGIT_DISTILL_FOLD_ROOT"
    return
  fi
  ...
  (( ${#found[@]} == 1 )) || die "Set LOGIT_DISTILL_FOLD_ROOT to the directory containing fold_assignments.parquet and fold_manifest.json"
  [[ -s "${found[0]}/fold_manifest.json" ]] || die "Missing fold_manifest.json beside ${found[0]}/fold_assignments.parquet"
  printf '%s\n' "${found[0]}"
}

FOLD_ROOT="$(find_fold_root)"
PREP_ROOT="${LOGIT_DISTILL_PREP_ROOT:-$ROOT/study_assets/preprocessing_p3_seed0_fold42}"
COMMON=(
  --config "$CONFIG"
...
check() {
  ...
  echo "[OK] P3 best settings: 30 epochs, buffer=4% per member (27778), replay=12.5%, cap=3"
  echo "[OK] loss: hybrid_logit_distill (focal current + CE replay + old-logit KL; feature MSE=OFF)"
  echo "[OK] member=0 only; output: $OUT"
}

start() {
  check
  [[ ! -e "$OUT/member_0" ]] || die "Output already exists: $OUT/member_0; choose a new LOGIT_DISTILL_OUT for a fresh pilot."
  mkdir -p "$MONITOR"
  ...
  nohup bash -c '
    ...
    env CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 \
      "$python_bin" scripts/visualize_cil_run.py \
        --run-root "$run_root" --outdir "$run_root/visualizations_member0" \
        --member-id 0 --member-ids 0 --split test
  ...
  echo "[LOG] $log/nohup.log (train xong sẽ tự tạo visualization_member0)"
}
...
status() {
  ...
  local log
  log="$(<"$MONITOR/latest.txt")"
  if kill -0 "$(<"$log/job.pid")" 2>/dev/null; then
    echo "[RUNNING] PID=$(<"$log/job.pid")"
  else
